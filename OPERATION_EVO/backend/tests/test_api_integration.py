...
class ApiIntegrationTestCase(unittest.TestCase):
    ...
    def test_authentication_and_crud_workflow(self):
        ...
        user_login = self.client.post(
            "/login",
            data={"email": "agent.api@example.com", "password": "Demo123!"},
            follow_redirects=False,
        )
        self.assertEqual(user_login.status_code, 302)
        self.assertIn("/dashboard", user_login.headers["Location"])
        ...
        self.assertEqual(created_ticket.get_json()["ticket"]["user_id"], user_id)

        self.assertEqual(self.client.post("/logout", json={}).status_code, 200)
        relogin_admin = self.login_admin()
        self.assertEqual(relogin_admin.status_code, 302)
        self.assertIn("/admin", relogin_admin.headers["Location"])
